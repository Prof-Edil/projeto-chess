...
--initial board ---
initial_board = Map.fromList [((1,1), Piece Black Rook), ((1,2), Piece Black Knight) , ((1,3), Piece Black Bishop)
...
testeBoard = Map.fromList $ zip [(1,2),(3,3),(5,4),(2,4),(1,4),(4,3)] [Piece Black Pawn, Piece White Rook, Piece Black Bishop, Piece White Queen, Piece Black King , Piece White Knight] :: Board
...
showMoves (x, y) board =  putStrLn $ (auxShowMoves 1 1 (x,y) board) . Map.toList $ board 
    where auxShowMoves i j (x, y) board [] 
...
moveShow x y board = auxMoveShow $ move x y board
    where auxMoveShow (Just x) = showBoard x
